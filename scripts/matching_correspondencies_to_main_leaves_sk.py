...
known_dict={} # Use this dict to check later which accessions didn't appear
...
with open("known_clusters_MIBig_and_clusterTable_matched_to_main.txt",'w') as output_file:
	with open("/scratch/groups/khosla/Orphan_PKS/results/antismash_results/interesting_clusters/alldbs/clusterTableNonRedundantNonSimilarNonSequenceSimilar.alldbs.prot_len.correct_domains.txt",'r') as final_cluster_file:
        ...
        for n in range(1,len(final_cluster_list)):
            ...
            for m in range(0,len(addiitonal_clusters1)):
			    ...
			    if cluster_n[0] == "\"":
			    	cluster_n = cluster_n[1:-1]
			    if cluster_n[0:3] != "r1c":
			    	print "Not in the right format",cluster_n
			...
			if curr_cluster in known_dict:
				del known_dict[curr_cluster]
            if len(known_products)!=0:
	            output_file.write(main_accession+" "+main_cluster_n+"\t"+known_products[:-2]+"\n")

print "Printing the accesions that didn't appear",known_dict
